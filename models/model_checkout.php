...
class model_checkout extends Model
{
    ...
    function updateCreditCard($data,$orderId){
        ...
        $sql="update tbl_order  set pay_day=?,pay_month=?,pay_year=?,pay_card=?,pay_bank_name=?,transaction_number=?,pay_hour=?,pay_minute=?,pay_type=4 where id=?";
        $params=array($day,$month,$year,$creditcard,$bank,$transaction_number,$hour,$minute,$orderId);
        $this->doQuery($sql,$params);

        header('location:' . URL . 'payment_result/index/' . $orderId);
