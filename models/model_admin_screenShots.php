...
class model_admin_screenShots extends Model
{
    ...
    function addPicture($data,$files)
    {
        ...
        if ($uploadOk == 1) {
            ...
            $sql = "insert into tbl_screenshots (img,link,name_alt,place_image) VALUES (?,?,?,?)";
    ...
    function delete_picture($data)
    {
        $ids = $data['id'];
        $ids = implode(',',$ids);
        $sql = "DELETE FROM tbl_screenshots WHERE id IN (".$ids.")";
        $this->doQuery($sql);
    }
