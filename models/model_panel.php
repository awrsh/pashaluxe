...
class model_panel extends Model
{
    ...
    function __construct()
    {
        parent::__construct();
        Model::sessionInit();
        $this->userId = Model::sessionGet('phone_number');
    ...
    function getStat()
    {
        ...
        $result = $this->doSelect($sql, [$userId]);
        $Stat['user_code'] = sizeof($result);

        $sql = "select * from tbl_code where userId=? and used=1";
        $result = $this->doSelect($sql, [$userId]);
        $Stat['user_code_used'] = sizeof($result);

        $sql = "select * from tbl_code where userId=? and darsad=50";
        $result = $this->doSelect($sql, [$userId]);
        $Stat['user_code_dar100'] = sizeof($result);
    ...
    function editProfile($data)
    {
        ...
        $sql = "update tbl_user set family=?,code_meli=?,tel=?,mobile=?,tavalod=?,address=?,jensiat=?,khabarname=?,cart_number=?,bank_name=? where email=?";
        $params = [$data['family'], $data['code_meli'], $data['tel'], $data['mobile'], $date, $data['address'], $data['jensiat'], @$data['khabarname'],$data['cart_number'],$data['bank_name'],$userId];
        $this->doQuery($sql, $params);
