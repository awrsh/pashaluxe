...
class model_adminproduct extends Model
{
    ...
    function addproduct($data = [], $productId, $file = array())
    {

        $title = $data['title'];
        $latin_title = $data['latin_title'];
        $categoryId = $data['categoryId'];
        $price = $data['price'];
        $introduction = $data['introduction'];
        $discount = $data['discount'];
        $special = $data['special'];
        $time_special = $data['time_special'];
        $only_myshop = $data['only_myshop'];
        $seller = $data['seller'];
        $Product_location = $data['Product_location'];
        $Absence = $data['Absence'];
        $code_product = rand(0, 900000,);
        $tedad_mojood = $data['tedad_mojood'];
        $weight = $data['weight'];
        $pasha_bon = $data['pasha_bon'];
        $brand = $data['brand'];
        ...
        if ($productId == '') {
            ...
            $sql = "insert into tbl_product (title,latin_title,price,cat,introduction,discount,score_bon,brand,special,time_special,only_myshop,seller,Product_location,Absence,code_product,tedad_mojood,colors,garantee,weight) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $value = array($title, $latin_title, $price, $categoryId, $introduction, $discount,$pasha_bon,$brand, $special, $time_special, $only_myshop, $seller, $Product_location, $Absence, $code_product, $tedad_mojood, $colors, $garantees, $weight);
            $this->doQuery($sql, $value);
            $productId = parent::$conn->lastInsertId();

            mkdir('public/images/products/' . $productId . '/gallery/small', 0777, 1);
            mkdir('public/images/products/' . $productId . '/gallery/large', 0777, 1);

        } else {
            $sql = "update tbl_product set title=?,latin_title=?,price=?,cat=?,introduction=?,discount=?,score_bon=?,brand=?,special=?,time_special=?,only_myshop=?,seller=?,Product_location=?,Absence=?,tedad_mojood=?,colors=?,weight=?,garantee=? where id=?";
            $value = array($title, $latin_title, $price, $categoryId, $introduction, $discount,$pasha_bon,$brand, $special, $time_special, $only_myshop, $seller, $Product_location, $Absence, $tedad_mojood, $colors, $weight, $garantees, $productId);
            $this->doQuery($sql, $value);
        }
