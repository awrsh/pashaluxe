...
class model_adminorder extends Model
{
    ...
    function editOrder($orderId, $data)
    {
        ...
        $sql = "update tbl_order set address=?,code_posti=?,tel=?,mobile=?,city=?,pay=?,status=? where id=?";
        $this->doQuery($sql, array($address, $code_posti, $tel,$mobile,$city, $pay_status, $order_status, $orderId));
